...
internal sealed class ObjectPipeline<TInput, TOutput> : IPipeline<object?, object?>
{
    ...
    public async Task<object?> InvokeAsync(object? input, CancellationToken cancellationToken = default)
    {
        ...
        TOutput result = await _inner.InvokeAsync(typed, cancellationToken);
...
/// 타입을 안전하게 캐스팅하기 위해 내부에서 사용되는 Wrapper입니다.
/// </summary>
internal sealed class ObjectPipeline<TInput> : IPipeline<object?>
{
    private readonly IPipeline<TInput> _inner;

    public ObjectPipeline(IPipeline<TInput> inner)
        => _inner = inner;

    public async Task InvokeAsync(object? input, CancellationToken cancellationToken = default)
    {
        if (input is not TInput typed)
            throw new InvalidCastException(
                $"Pipeline expected input of type {typeof(TInput).FullName}, but got {input?.GetType().FullName ?? "null"}");

        await _inner.InvokeAsync(typed, cancellationToken);
    }
}
...
/// 함수(델리게이트)를 감싸는 파이프라인 Wrapper입니다.
/// </summary>
internal sealed class DelegatePipeline<TInput> : IPipeline<TInput>
{
    private readonly Func<TInput, CancellationToken, Task> _function;

    public DelegatePipeline(Func<TInput, CancellationToken, Task> function)
        => _function = function;

    public DelegatePipeline(Func<TInput, Task> function)
        : this((input, ct) => function(input)) { }

    public DelegatePipeline(Action<TInput> function)
        : this((input, ct) => { function(input); return Task.CompletedTask; }) { }
...
/// DI 컨테이너에서 파이프라인을 생성해서 사용되는 Wrapper입니다.
/// </summary>
internal sealed class ServiceFactoryPipeline<TInput> : IPipeline<TInput>
{
    private readonly IServiceProvider _services;
    private readonly Func<IServiceProvider, IPipeline<TInput>> _factory;
...
internal sealed class ServiceFactoryPipelineHook : IPipelineHook
{
    ...
    public ValueTask CancelAsync(string name, CancellationToken cancellationToken = default)
