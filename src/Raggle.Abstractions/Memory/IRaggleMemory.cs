...
public interface IRaggleMemory
{
    ...
    /// <param name="filter">적용할 필터 조건</param>
    /// <param name="cancellationToken">작업 취소를 위한 토큰</param>
    Task SearchDocumentMemoryAsync(string collectionName, string query, float minScore = 0, int limit = 5, MemoryFilter? filter = null, CancellationToken cancellationToken = default);
}
